//-------------------------------------------------------------------
//	myshowgdt.c  获取GDT中各个段的段描述
//	本内核文件创建一个proc伪文件，'/proc/myshowgdt'	
//  编译命令： $make 
//  内核模块添加：$sudo insmod myshowgdt.ko  
//  显示信息：$cat /proc/myshowgdt
//  内核模块删除：$sudo rmmod myshowgdt
// 注意：物理地址、进程用户空间地址、进程内核空间地址之间的关系和转换
// 注意：物理内存与虚拟内存的关系
//-------------------------------------------------------------------
